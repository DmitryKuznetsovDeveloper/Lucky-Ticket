...
public class PurchaseHandler :  IDisposable
{
    ...
    private FastTicketConfigs _config;

    public PurchaseHandler(CurrencyManager currencyManager, TicketView ticketView, SimpleScratch simpleScratch, int ticketCost, int rewardAmount, InfiniteCarousel carousel)
    {
    ...
    {
        _ticketView.Button.BaseButton.onClick.AddListener(()=> _ticketView.Button.PlayOnClick(OnBuyTicket));
        _carousel.OnCenterConfigChanged += GeCurrentConfig;
    }

    private void GeCurrentConfig(FastTicketConfigs config) => _config = config;
